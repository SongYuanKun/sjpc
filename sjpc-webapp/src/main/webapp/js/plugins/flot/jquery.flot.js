...
(function($) {
	function Plot(placeholder, data_, options_, plugins) {
		...
		var series = [], options = {
			...
			// how far the mouse can be away to
			// activate an item
			},
			hooks : {}
		...
		function parseOptions(opts) {
			...
			for (i = 0; i < Math.max(1, options.xaxes.length); ++i)
				options.xaxes[i] = $.extend(true, {}, options.xaxis,
						options.xaxes[i]);
			for (i = 0; i < Math.max(1, options.yaxes.length); ++i)
				options.yaxes[i] = $.extend(true, {}, options.yaxis,
						options.yaxes[i]);
		...
		function setupCanvases() {
			var reused, existingCanvas = placeholder.children("canvas.base"), existingOverlay = placeholder
		...
		function measureTickLabels(axis) {
			...
			function makeDummyDiv(labels, width) {
				return $(
						'<div style="position:absolute;top:-10000px;' + width
								+ 'font-size:smaller">' + '<div class="'
								+ axis.direction + 'Axis ' + axis.direction
								+ axis.n + 'Axis">' + labels.join("")
			...
			if (axis.direction == "x") {
				...
				if (w == null)
					w = Math.floor(canvasWidth
							/ (ticks.length > 0 ? ticks.length : 1));
		...
		function setupGrid() {
			...
			if (options.grid.show) {
				...
				if (minMargin == null) {
					minMargin = 0;
					for (i = 0; i < series.length; ++i)
						minMargin = Math.max(minMargin, series[i].points.radius
		...
		function setupTickGeneration(axis) {
			...
			if (typeof opts.ticks == "number" && opts.ticks > 0)
				...
				noTicks = 0.3 * Math.sqrt(axis.direction == "x" ? canvasWidth
						: canvasHeight);
			...
			if (opts.mode == "time") {
				...
				formatter = function(v, axis) {
					...
					if (opts.timeformat != null)
						return $.plot.formatDate(d, opts.timeformat,
								opts.monthNames);
			...
			if (opts.alignTicksWithAxis != null) {
				...
				if (otherAxis && otherAxis.used && otherAxis != axis) {
					...
					if (axis.mode != "time" && opts.tickDecimals == null) {
						var extraDec = Math.max(0, -Math.floor(Math.log(delta)
								/ Math.LN10) + 1), ts = generator(axis);
		...
		function drawGrid() {
			...
			for (var j = 0; j < axes.length; ++j) {
				...
				ctx.strokeStyle = axis.options.tickColor
						|| $.color.parse(axis.options.color).scale('a', 0.22)
								.toString();
				ctx.lineWidth = 1;
		...
		function insertAxisLabels() {
			...
			for (var j = 0; j < axes.length; ++j) {
				...
				html.push('<div class="' + axis.direction + 'Axis '
						+ axis.direction + axis.n + 'Axis" style="color:'
						+ axis.options.color + '">');
				for (var i = 0; i < axis.ticks.length; ++i) {
					...
					if (axis.direction == "x") {
						align = "center";
						pos.left = Math.round(plotOffset.left
								+ axis.p2c(tick.v) - axis.labelWidth / 2);
						if (axis.position == "bottom")
							pos.top = box.top + box.padding;
						else
							pos.bottom = canvasHeight
					...
					html.push('<div class="tickLabel" style="'
							+ style.join(';') + '">' + tick.label + '</div>');
				}
				html.push('</div>');
			}
